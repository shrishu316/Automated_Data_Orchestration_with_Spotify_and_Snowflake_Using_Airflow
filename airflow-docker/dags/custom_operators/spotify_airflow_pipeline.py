...
def _read_data_from_s3(**kwargs):
    ...
    bucket_name = "spotify-project-shubham"
...
def _process_album(**kwargs):
    ...
    for data in spotify_data:
        for row in data ['items']:
            ...
            'total_tracks':album_total_tracks,'uri':album_uri}
...
def _process_songs(**kwargs):
    ...
    for data in spotify_data:
        for row in data ['items']:
            ...
            songs_uri = row ['track'] ['uri']
            album_id = row ['track'] ['album'] ['id']
            artist = row ['track'] ['album'] ['artists'][0]['id'] 
            song_dict={'song_id':song_id, 'song_name': song_name, 'song_track_number': song_track_number, 'duration': song_duration_ms,
            'popularity':songs_popularity, 'song_uri':songs_uri,'song_added_at':song_added, 'album_id':album_id, 'artist_id':artist}
...
store_raw_to_s3 = S3CreateObjectOperator(
    ...
    s3_bucket="spotify-project-shubham",
